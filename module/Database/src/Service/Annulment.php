...
use Database\Model\SubDecision\FoundationReference as FoundationReferenceModel;
...
class Annulment
{
        ...
        foreach ($target->getSubdecisions() as $subDecision) {
            foreach ($this->findRelated($subDecision) as $related) {
                ...
                if ($this->isAfter($related, $annulment)) {
                    throw $this->cannot($subDecision, $this->translator->translate(
                ...
                $laterAnnulments = $this->meetingMapper->findReferencingSubDecisions(
     ...
     * live on {@see \Application\Model\Enums\OrganTypes}. Taking a decision back can break them just as easily as
    ...
    private function findActiveInstallations(
        ...
        $installations = [];
        $references = $this->meetingMapper->findReferencingSubDecisions(
        ...
        foreach ($references as $reference) {
            ...
            foreach (
                $this->meetingMapper->findReferencingSubDecisions(
    ...
    private function findOrganViolations(
        ...
        if (
            ...
            $violations['chair'] = sprintf($this->translator->translate('it would leave %s without a chair'), $abbr);
        }
        ...
        if ($members < $minimum) {
            $violations['minimum-members'] = sprintf(
                $this->translator->translate('it would leave %s with fewer than %d active member(s)'),
                $abbr,
                $minimum,
            );
        }
        ...
        switch (true) {
            case $subDecision instanceof FoundationModel:
                ...
                $this->assertNone(
                    ...
                    $subDecision,
                    $this->translator->translate('the organ is still referenced by other decisions'),
                );
                break;
            ...
            case $subDecision instanceof AbrogationModel:
                ...
                $this->assertNone(
                    ...
                    $subDecision,
                    $this->translator->translate('the organ was also abolished by another decision'),
                );
                $this->assertNone(
                    array_filter(
                    ...
                    $subDecision,
                    $this->translator->translate('members were installed in the organ after it was abolished'),
                );
                break;
            ...
            case $subDecision instanceof InstallationModel:
                $this->assertNone(
                    ...
                    $subDecision,
                    $this->translator->translate('the member was discharged after being installed'),
                );
                $this->assertNone(
                    $this->findDependents(ReappointmentModel::class, 'installation', $subDecision, $subDecision),
                    $subDecision,
                    $this->translator->translate('the installation was prolonged after it was made'),
                );
                break;
            ...
            case $subDecision instanceof ReappointmentModel:
                ...
                $warnings = [
                    ...
                    ...$this->warnAbout(
                        ...
                        $subDecision,
                        $this->translator->translate('the member was discharged after being reappointed'),
                    ),
                    ...$this->warnAbout(
                        array_filter(
                            $this->findDependents(
                        ...
                        $subDecision,
                        $this->translator->translate('the installation was prolonged again afterwards'),
                    ),
                ];
                break;
            ...
            case $subDecision instanceof DischargeModel:
                $this->assertNone(
                    ...
                    $subDecision,
                    $this->translator->translate('the member was also discharged by another decision'),
                );
                break;
            ...
            case $subDecision instanceof BoardInstallationModel:
                $this->assertNone(
                    ...
                    $subDecision,
                    $this->translator->translate('the board member was released after being installed'),
                );
                $this->assertNone(
                    $this->findDependents(BoardDischargeModel::class, 'installation', $subDecision, $subDecision),
                    $subDecision,
                    $this->translator->translate('the board member was discharged after being installed'),
                );
                break;
            ...
            case $subDecision instanceof BoardReleaseModel:
                ...
                $this->assertNone(
                    ...
                    $subDecision,
                    $this->translator->translate('the board member was also released by another decision'),
                );
                $this->assertNone(
                    $this->findDependents(BoardDischargeModel::class, 'installation', $installation, $subDecision),
                    $subDecision,
                    $this->translator->translate('the board member was discharged after being released'),
                );
                break;
            ...
            case $subDecision instanceof BoardDischargeModel:
                $this->assertNone(
                    ...
                    $subDecision,
                    $this->translator->translate('the board member was also discharged by another decision'),
                );
                break;
            ...
            case $subDecision instanceof KeyGrantingModel:
                $this->assertNone(
                    ...
                    $subDecision,
                    $this->translator->translate('the key code was withdrawn after it was granted'),
                );
                break;
            ...
            case $subDecision instanceof KeyWithdrawalModel:
                $this->assertNone(
                    ...
                    $subDecision,
                    $this->translator->translate('the key code was also withdrawn by another decision'),
                );
                break;
        }
    ...
    private function cannot(
        ...
        return new AnnulmentNotPossible(sprintf(
            $this->translator->translate('Decision %s %d.%d.%d cannot be annulled, because %s.'),
            $decision->getMeetingType()->value,
            $decision->getMeetingNumber(),
            $decision->getPoint(),
            $decision->getNumber(),
        ...
        switch (true) {
            ...
            case $anchor instanceof BoardInstallationModel:
                foreach ([BoardReleaseModel::class, BoardDischargeModel::class] as $type) {
                    $related = [
                        ...$related,
                        ...$this->meetingMapper->findReferencingSubDecisions($type, 'installation', $anchor),
                    ];
                }
    ...
    private function findInstallationReferences(InstallationModel $installation): array
    {
        ...
        foreach ([DischargeModel::class, ReappointmentModel::class] as $type) {
            $references = [
                ...$references,
                ...$this->meetingMapper->findReferencingSubDecisions($type, 'installation', $installation),
            ];
        }
    ...
    private function findDependents(
        ...
        return array_values(array_filter(
            $this->meetingMapper->findReferencingSubDecisions($type, $property, $referenced),
    ...
    {
        return [] !== $this->meetingMapper->findReferencingSubDecisions(
            AnnulmentModel::class,
            'target',
            $decision,
        );
