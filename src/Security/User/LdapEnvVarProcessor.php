...
/**
 * Reshapes the LDAP environment, which is still in the shape laminas-ldap wanted, into what ext_ldap and the login
 * firewall expect. Doing it here rather than asking for new values keeps existing deployments' environment working:
 * every one of these translations would otherwise fail silently, as an unusable filter or an unreachable server.
 */
final class LdapEnvVarProcessor implements EnvVarProcessorInterface
{
    ...
    public function getEnv(
        ...
        return match ($prefix) {
            ...
            // `{user_identifier}`.
            'ldap_filter' => str_replace('%s', '{user_identifier}', $value),
            ...
            'ldap_encryption' => true === filter_var($value, FILTER_VALIDATE_BOOL, FILTER_NULL_ON_FAILURE)
                ? 'tls'
                : 'none',
    ...
    {
        $hosts = array_filter(
            array_map(trim(...), explode(',', $servers)),
            static fn (string $host): bool => '' !== $host,
        );

        return implode(
            ' ',
            array_map(static fn (string $host): string => 'ldap://' . $host . ':' . self::LDAP_PORT, $hosts),
        );
    }
}
