...
/**
 * The paging every overview does the same way: a page number, a clamped page size, the totals the pagination
 * partial renders, and one query per request.
 *
 * Subclasses answer {@see self::createPaginator()} with their own filtered query and keep their own filter props.
 * `#[AsLiveComponent]` stays on the concrete component, since each overview is gated differently.
 *
 * A subclass must bind the type it pages over, or static analysis rejects its createPaginator() against this one:
 *
